...
class App {
	...
	drawImage() {
		...
		// 이미지 최대위치? = 배경 너비와 높이? (우하단? 가리키는 듯?)
		this.imgPos.width = this.stageWidth;
		this.imgPos.height = this.stageHeight;

		// 이미지 비율이 배경 비율보가 크면(가로가 더 길면)
		if (imgRatio > stageRatio) {
			// 이미지 가로 위치는 이미지 너비에 배경높이/이미지높이 비율을 곱
	...
	drawDots() {
		...
		for (let i = 0; i < this.rows; i++) {
			...
			const pixelY = Math.max(Math.min(y, this.stageWidth), 0);

			for (let j = 0; j < this.columns; j++) {
				const x = (j + 0.5) * this.pixelSize;
				const pixelX = Math.max(Math.min(x, this.stageWidth), 0);

				const pixelInex = (pixelX + pixelY * this.stageWidth) * 4;
				const red = this.imgData.data[pixelInex + 0];
